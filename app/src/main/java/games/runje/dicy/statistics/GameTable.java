...
public class GameTable
{
    ...
    private static final String[] Player = new String[]{
            KEY_ID, KEY_P1, KEY_P2, KEY_P1Start, KEY_P1Won, KEY_P1Points, KEY_P2Points, KEY_DATE, KEY_LENGTH, KEY_RULEVARIANT};
    ...
    private static ContentValues gameToValues(GameStatistic game)
    {
        ...
        values.put(KEY_RULEVARIANT, game.getRuleVariant().toString());
        return values;
    }
    ...
    private static GameStatistic createGameFromCursor(Cursor cursor)
    {
        try
        {
            return new GameStatistic(cursor.getLong(0), cursor.getString(1), cursor.getString(2), cursor.getInt(3) == 1, cursor.getInt(4) == 1, cursor.getInt(5), cursor.getInt(6), new Date(cursor.getLong(7)), GameLength.valueOf(cursor.getString(8)), RuleVariant.getEnum(cursor.getString(9)));
        } catch (Exception e)
        {
            // old db
            return new GameStatistic(cursor.getLong(0), cursor.getString(1), cursor.getString(2), cursor.getInt(3) == 1, cursor.getInt(4) == 1, cursor.getInt(5), cursor.getInt(6), new Date(cursor.getLong(7)), GameLength.valueOf(cursor.getString(8)), RuleVariant.Atlantic_City);
        }
    }
    ...
    public static List<GameStatistic> getGames(SQLiteDatabase db, GameLength length, RuleVariant ruleVariant)
    {
        ArrayList<GameStatistic> games = new ArrayList<>();

        String selectQuery = "SELECT * FROM " + TABLE_GAMES + " WHERE (" + KEY_LENGTH + " = ? AND " + KEY_RULEVARIANT + " = ?)";

        Cursor cursor = db.rawQuery(selectQuery, new String[]{length.toString(), ruleVariant.toString()});
